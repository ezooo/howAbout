...
<body>
	...
	%>
		<div id="createDiv">
			<div style="background-image: url('/howAbout/resources/userIcon/<%= member.getIconName() %>');" id="userImg"></div>
			<form:form id="joinForm" method="post" modelAttribute="member" enctype="multipart/form-data">
				<div class="memberInputDiv">
					<p class="form_tag">닉네임 </p>
					<form:input value="<%= member.getUserName() %>" class="formInput" path="userName"/>
				</div>
				<div class="memberInputDiv">
					<p class="form_tag">아이디 </p>
					<form:input value="<%= member.getUserId() %>"  class="formInput" id="userId" path="userId"/>
					<!-- <button type="button" id="matchTheID" onclick="sendToController('userId')" >아이디 중복검사</button>  -->
				</div>
				<div class="memberInputDiv">
					<p class="form_tag">비밀번호 </p>
					<form:input value="<%= member.getUserPw() %>" class="formInput" path="userPw"/>
				</div>
				<div class="memberInputDiv">
					<p class="form_tag">전화번호 </p>
					<form:input value="<%= member.getUserTel() %>" class="formInput" path="userTel"/>
				</div>
				<div class="memberInputDiv" id="memberAddrForm">
					<p class="form_tag">주소 </p><form:input value="<%= member.getUserAddr() %>" placeholder="위치별 정보를 전해드립니다, 드롭박스 안에서 선택해주세요 (세부주소 X)" class="formInput" path="userAddr" type="text" id="locationInput" onkeyup="searchLocation()" />
					<div id="results" class="results-container"></div>
				</div>
				
				<div class="memberInputDiv">
					<p class="form_tag">이메일 </p>
					<form:input value="<%= member.getUserEmail() %>" class="formInput" id="userEmail" path="userEmail"/>
